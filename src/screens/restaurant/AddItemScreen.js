...
import {EXPO_PUBLIC_AZURE_CONTAINER, EXPO_PUBLIC_AZURE_SAS_TOKEN, EXPO_PUBLIC_AZURE_STORAGE_ACCOUNT} from '@env';

// Azure Blob Storage configuration
const AZURE_STORAGE_ACCOUNT = process.env.EXPO_PUBLIC_AZURE_STORAGE_ACCOUNT || 'your_storage_account';
const AZURE_CONTAINER = process.env.EXPO_PUBLIC_AZURE_CONTAINER || 'menu-images';
const AZURE_SAS_TOKEN = process.env.EXPO_PUBLIC_AZURE_SAS_TOKEN || 'your_sas_token';
...
export default function AddItemScreen({ navigation }) {
  ...
  const uploadToAzure = async (uri) => {
    try {
      ...
      const filename = `dish_${user.uid}_${Date.now()}.jpg`;
      ...
      if (uploadResponse.ok) {
        // Generate public URL without SAS token
